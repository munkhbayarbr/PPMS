...
const fetcher = (url: string) => apiFetch<FiberColor[]>(url);

export default function FiberColorsPage() {
  const { data, isLoading, error, mutate } = useSWR<FiberColor[]>("/fiber-colors", fetcher);
  const [open, setOpen] = useState(false);
  const [form, setForm] = useState<Partial<FiberColor>>({ name: "" });
  ...
  async function submit() {
    try {
      ...
      if (editingId) {
        await apiFetch(`/fiber-colors/${editingId}`, { method: "PATCH", body: JSON.stringify({ name: form.name }) });
      } else {
        await apiFetch(`/fiber-colors`, { method: "POST", body: JSON.stringify({ name: form.name }) });
      }
      toast.success("OK");
      close(); mutate();
  ...
  async function remove(id: string) {
    try { await apiFetch(`/fiber-colors/${id}`, { method: "DELETE" }); mutate(); }
    catch(e:any){ toast.error(e?.message || "Амжилтгүй"); }
  }
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold">Ширхэгийн өнгө (Fiber Colors)</h1>
      ...
      <div className="rounded-xl border">
        <div className="p-3 border-b text-sm text-muted-foreground">
          {isLoading ? "Ачаалж байна…" : error ? "Алдаа" : `${data?.length ?? 0} өнгө`}
        </div>
