...
export class CreateP4SpinningDto {
  ...
  @ApiProperty({ required: false }) @IsOptional() @IsNumber() p4DanUtas?: number;     // single yarn
  @ApiProperty({ required: false }) @IsOptional() @IsNumber() p4RovenWeight?: number;
  @ApiProperty({ required: false }) @IsOptional() @IsNumber() p4Waste?: number;
